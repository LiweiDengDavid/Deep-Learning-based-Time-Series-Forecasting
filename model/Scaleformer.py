...
class Scaleformer(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.enc_embedding = DataEmbedding_mine(configs.d_feature, configs.d_model, configs.embed, configs.freq, configs.dropout)
        self.dec_embedding = DataEmbedding_mine(configs.d_feature, configs.d_model, configs.embed, configs.freq, configs.dropout, is_decoder=True)
        ...
        self.scales = count_scale(self.scale_factor,self.seq_len)
        self.mv = moving_avg()
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        outputs = []
        for scale in scales:
            enc_out = self.mv(x_enc, scale) # 根据所给scale对原始序列进行均值滤波
            ...
            dec_out = self.dec_embedding(seasonal_init, x_mark_dec[:, scale//2::scale][:,:seasonal_init.shape[1]], scale=scale, first_scale=scales[0], label_len=label_len)
...
def count_scale(factor,seqlen):
    m = int(np.log(seqlen)/np.log(factor))-1
    return [factor**i for i in range(m-1,0-1,-1)]
